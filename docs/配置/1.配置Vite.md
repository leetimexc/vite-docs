...
问题：当你在 VS Code 中调试 Vite 配置文件时，设置的断点会失效，提示"文件未找到"。
‌

#### 为什么会这样？‌

- 官方解释：在默认的 `--configLoader bundle` 行为下，Vite 会将生成的临时配置文件写入 `node_modules/.vite-temp` 文件夹，在 Vite 配置文件中设置断点调试时会出现文件未找到的错误。

- 通俗解释：因为 Vite 把配置文件复制到了一个临时文件夹（node_modules/.vite-temp），但 VS Code 不知道要去这个临时文件夹找文件。

#### 解决方案很简单：‌

1. 在项目根目录找到或创建 .vscode 文件夹
2. 在这个文件夹里创建 settings.json 文件
3. 把下面这段代码复制进去：
...
- 告诉 VS Code："除了正常的工作区文件，请特别关注 node_modules/.vite-temp 这个文件夹"
- 这样断点就能正确连接到临时文件了

**简单说就是：加个配置，让 VS Code 知道去哪找文件，断点就能正常工作了！**
